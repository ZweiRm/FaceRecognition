...
# 导入数据集
data = open('./datasets/Yale_64x64.mat', 'rb')
...
# 划分 80%(132)训练； 20%(33)测试
train_data = feature[0:132]
train_label = label[0:132]

test_data = feature[132:]
test_label = label[132:]
...
callback = [
    # keras.callbacks.ModelCheckpoint(output_model_file, save_best_only=True, monitor='accuracy'),
...
def cnn_model():
    ...
    model.add(layers.Conv2D(filters=128, kernel_size=(8, 8), activation=tf.nn.relu, padding='SAME', input_shape=(64, 64, 1)))
    model.add(layers.MaxPool2D(pool_size=(4, 4), strides=(4, 4), padding='SAME'))
    model.add(layers.Dropout(0.05))

    # 第二个卷积块和池化层
    model = tf.keras.models.Sequential()
    model.add(layers.Conv2D(filters=16, kernel_size=(8, 8), activation=tf.nn.relu, padding='SAME'))
    model.add(layers.MaxPool2D(pool_size=(4, 4), strides=(4, 4), padding='SAME'))
    model.add(layers.Dropout(0.05))
    ...
    # 输出层
    model.add(layers.Dense(units=15, activation=tf.nn.softmax))
    model.add(layers.Dropout(0.05))
