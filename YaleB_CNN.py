...
# 导入数据集
data = open('./datasets/YaleB_32x32.mat', 'rb')
...
# 划分 80%(1931)训练； 20%(483)测试
train_data = feature[0:1931]
train_label = label[0:1931]

test_data = feature[1931:]
test_label = label[1931:]

# 重构数据集
train_data = train_data.reshape(train_data.shape[0], 32, 32, 1).astype(np.float32) / 255
for i in range(0, train_label.shape[0]):
    train_label[i] -= 1

test_data = test_data.reshape(test_data.shape[0], 32, 32, 1).astype(np.float32) / 255
for i in range(0, test_label.shape[0]):
    test_label[i] -= 1

# 记录器
class_names = numpy.arange(0, 38, 1)
class_names = np.array(class_names)
logdir = './tensorboard_YaleB_CNN'
...
def image_grid():
  ...
  for i in range(1, 26):
    ...
    plt.yticks([])
    plt.grid(False)
    images = np.reshape(feature[i], (32, 32))
    images = images.T
...
def cnn_model():
    ...
    model.add(
        layers.Conv2D(filters=128, kernel_size=(4, 4), activation=tf.nn.relu, padding='SAME', input_shape=(32, 32, 1)))
    model.add(layers.MaxPool2D(pool_size=(2, 2), strides=(2, 2), padding='SAME'))
    model.add(layers.Dropout(0.05))

    # 第二个卷积块和池化层
    model = tf.keras.models.Sequential()
    model.add(layers.Conv2D(filters=128, kernel_size=(4, 4), activation=tf.nn.relu, padding='SAME'))
    model.add(layers.MaxPool2D(pool_size=(2, 2), strides=(2, 2), padding='SAME'))
    model.add(layers.Dropout(0.05))
